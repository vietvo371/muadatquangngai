...
/** POST /api/v2/banners/[slug]/view — port của BannerController@trackView (tăng view_count theo slug). */
export async function POST(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  await db.banners.update({ where: { id: banner.id }, data: { view_count: { increment: 1 } } });
