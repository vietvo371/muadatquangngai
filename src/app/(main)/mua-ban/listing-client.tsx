...
function PropertyListingContent() {
  ...
  // CHỈ hiển thị tin THẬT từ API. Trước đây khi API lỗi/rỗng thì rơi về `mockProperties` —
  // khách thấy tin bịa như tin thật; thà hiện "không tìm thấy tin đăng" còn hơn.
        ...
        <div className={`flex gap-6 items-start ${mapMode ? 'h-[calc(100vh-150px)]' : ''}`}>
          ...
          <div className={`min-w-0 ${mapMode ? 'lg:w-[40%] lg:flex-none w-full h-full overflow-y-auto pr-1' : 'flex-1'}`}>
            ...
            {/* Danh sách BĐS — một giao diện lưới thống nhất (đã bỏ toggle Lưới/Danh sách).
                Khi bật bản đồ, danh sách co về 1 cột cho vừa cột trái hẹp. */}
