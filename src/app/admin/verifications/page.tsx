...
export default function AdminVerificationsPage() {
  ...
  // Fetch verifications from real API or fallback to mock data
  const loadVerifications = useCallback(async () => {
    try {
      setLoading(true);
      const params: Record<string, string> = {};
      if (statusFilter !== 'all') params.type = statusFilter; // matching original API query key "type" for status
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      ...
      // Update local state directly for seamless experience
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      setIsActionPending(true);
      if (useRealApi) {
        await verificationApi.reject(selectedVerification.id, rejectReason);
      }
      setVerifications((prev) =>
        prev.map((v) =>
          v.id === selectedVerification.id
            ? { ...v, status: 'rejected', reject_reason: rejectReason }
            : v
        )
      );
