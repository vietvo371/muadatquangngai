...
export function BoostModal({ open, onOpenChange, propertyId, propertyTitle }: BoostModalProps) {
  ...
  const { data: boostInfo, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/properties/${propertyId}/boost`);
  ...
  const boostMutation = useMutation({
    mutationFn: async (tier: string) => {
      const res = await api.post(`/properties/${propertyId}/boost`, { tier });
      return res.data;
    },
    onSuccess: (data) => {
  ...
  const renewMutation = useMutation({
    mutationFn: async () => {
      const res = await api.post(`/properties/${propertyId}/renew`);
      return res.data;
    },
    onSuccess: (data) => {
